        ...
        <div ref={ref} className={`flex items-end ${msg.sender === 'me' ? 'justify-end' : 'justify-start'} mb-2`}>
            {msg.sender !== 'me' && (
                <img
                    src={avatarSrc}
                    alt={avatarAlt}
                    className="h-6 w-6 md:h-8 md:w-8 rounded-full object-cover mr-2"
                />
            )}

            <div className={`relative max-w-[75%] md:max-w-[50%] rounded-2xl px-3 md:px-4 py-2 break-words flex flex-col ${msg.sender === 'me' ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100'}`}>
            ...
            {msg.sender === 'me' && (
                <img
                    src={avatarSrc}
                    alt={avatarAlt}
                    className="h-6 w-6 md:h-8 md:w-8 rounded-full object-cover ml-2"
                />
            )}
        </div>
    );
