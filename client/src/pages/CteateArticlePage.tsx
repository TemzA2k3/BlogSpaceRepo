...
export const CreateArticlePage = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (!content.trim()) {
            setValidationError("Нужно добавить текст статьи");
            return;
        }

        if (!coverImage) {
            setValidationError("Нужно добавить обложку статьи");
            return;
        }
        ...
        <div className="max-w-[56rem] mx-auto mt-8 px-2 pb-8">
                    ...
                    <div className="px-6 pb-6 flex flex-wrap gap-2">
                        {tags.map((tag, idx) => (
                            <HashTag
                                key={idx}
                                tag={tag}
                                onRemove={() => setTags((prev) => prev.filter((_, i) => i !== idx))}
                            />
                        ))}
                    </div>
                )}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-3">
                <button
                    ...
                    className="w-full sm:w-auto rounded-full bg-gray-200 dark:bg-gray-700 px-6 py-2 text-gray-700 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600 transition"
                >
                    Отменить
                </button>
                ...
                <button
                    ...
                    className="w-full sm:w-auto rounded-full bg-blue-500 px-6 py-2 text-white hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 whitespace-nowrap transition-all duration-200"
                >
