...
export const ProfilePage = () => {
  ...
  const [userData, setUserData] = useState<User & { isFollowing?: boolean } | null>(null);
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showAlert(err.message || "Failed to update avatar", "error");
    }
  };
    ...
    if (!userData) return;
      ...
      if (isFollowing) {
        unfollowUser(userData.id)
            ...
            .catch(err => showAlert(err.message || "Failed to unfollow this user", "error"))
  ...
  return (
    <main className="max-w-5xl mx-auto py-10 px-4 sm:px-6 lg:px-8 text-gray-800 dark:text-gray-100">
      {/* Шапка профиля */}
      <section
        className={`border rounded-2xl p-6 shadow-sm mb-8 ${
          isMyProfile
            ...
            {isMyProfile ? (
              ...
              <div className="mt-6 flex gap-3 justify-center sm:justify-start">
                <button
                  ...
                  className={`px-5 py-2 rounded-xl font-medium transition flex items-center justify-center gap-2 ${
                    isFollowing
